...
- **테이블(표)**: `{ type: 'table', minWidth: '800px', headers: ['컬럼1', '컬럼2'], rows: [['내용1', '내용2'], ...] }` 객체를 사용합니다. (`minWidth`는 선택사항이며, 내용이 넓을 경우 가로 스크롤을 유도합니다)
- **이미지 첨부**: `{ type: 'image', src: '/이미지경로.png', alt: '대체 텍스트', caption: '캡션 내용(선택)', maxWidth: '100%(선택)' }` 객체를 사용합니다. (이미지 파일은 `public` 폴더 내에 위치해야 합니다)
- **토글 내 소제목**: 토글의 content 안에서 `{ type: 'subTitle', text: '소제목' }` 을 사용합니다.
- **하위 계층 지원 글머리 기호(Nested Bullet)**: 글머리 기호 아래에 또 다른 토글이나 코드를 들여쓰기하여 넣으려면 `{ type: 'bullet', text: '본문', children: [...] }` 객체를 사용합니다. (노션의 계층 구조와 동일)
